...
class mainSpecBase {
	constructor(config) {
		...
		config.utilPath = path.resolve(process.cwd(), config.utilPath || "");
     ...
     */
	executeCommonTest() {
		let { width, height, ua, addr, headless, configPath, utilPath } = this.config;
     ...
     */
	executeE2ETest() {
		let { width, height, ua, addr, configPath, headless, e2eCases,utilPath } = this.config;
